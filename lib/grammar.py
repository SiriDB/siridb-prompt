"""
 ...
 Created at: 2016-10-24 13:50:41
"""
import re
...
class SiriGrammar(Grammar):
    ...
    r_doubleq_str = Regex('^(?:"(?:[^"]*)")+')
    r_grave_str = Regex('^(?:`(?:[^`]*)`)+')
    r_uuid_str = Regex('^[0-9a-f]{8}\\-[0-9a-f]{4}\\-[0-9a-f]{4}\\-[0-9a-f]{4}\\-[0-9a-f]{12}')
    r_regex = Regex('^(/[^/\\\\]*(?:\\\\.[^/\\\\]*)*/i?)')
    r_comment = Regex('^#.*')
    ...
    series_match = List(Choice(
        series_name,
        group_match,
        series_re,
        most_greedy=False), series_sep, 1, None, False)
    limit_expr = Sequence(
        k_limit,
        int_expr
    )
    before_expr = Sequence(
        k_before,
        time_expr
    )
    after_expr = Sequence(
    ...
    prefix_expr = Sequence(
        k_prefix,
        string
    )
    suffix_expr = Sequence(
        k_suffix,
        string
    )
    f_points = Choice(
    ...
    set_address = Sequence(
        k_set,
        k_address,
        string
    )
    set_backup_mode = Sequence(
        k_set,
        k_backup_mode,
        _boolean
    )
    set_drop_threshold = Sequence(
        k_set,
        k_drop_threshold,
        r_float
    )
    set_expression = Sequence(
        k_set,
        k_expression,
        r_regex
    )
    set_ignore_threshold = Sequence(
        k_set,
        k_ignore_threshold,
        _boolean
    )
    ...
    select_stmt = Sequence(
        k_select,
        List(select_aggregate, Token(','), 1, None, False),
        k_from,
        series_match,
        Optional(where_series),
        Optional(Choice(
